...
1. Qual padrão de projeto pode ser utilizado para resolver este problema? Implemente a solução (não precisa enviar mensagens de fato, apenas simular com prints).
    ...
    O padrão de projeto que pode ser utilizado é o **Factory Method**. Ele permite encapsular a lógica de criação de objetos, facilitando a troca ou adição de novas classes de envio de mensagens sem modificar o código existente. A implementação pode ser feita criando uma interface `MessageService` e classes concretas como `Email`, `Sms`, etc.
